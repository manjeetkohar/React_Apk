...
export default function AddUserForm(formData, setFormData) {
    ...
    // first item in array in next line is initial state and second item is the state we want to set 
    ...
    if (addMutation.isSuccess) return <Success message={"add user successfull"}></Success>
    ...
    return (
        <form className="grid lg:grid-cols-2 w-4/6 gap-4" onSubmit={handleSubmit}>
            ...
            <button type="submit" className="flex justify-center text-md w-2/6 bg-green-500 text-white px-4 py-2 border rounded-md hover:bg-gray-50 hover:border-green-500 hover:text-green-500">Add <span className="px-1"><BiPlus size={24}></BiPlus> </span></button>
        </form>
    )
}